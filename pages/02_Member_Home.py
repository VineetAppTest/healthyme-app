...
def _render_member_home_css():
    st.markdown(
        """
<style>
/* Member Home only: compact top placement and balanced two-column cards. */
...
@media(max-width:900px){
  div[data-testid="stHorizontalBlock"]:has(.hm-member-home-balanced-card) > div[data-testid="column"]{display:block!important;}
}
</style>
""",
...
def _render_messages(user_id):
    ...
    if not messages:
        return
    st.markdown(
        "<div class='hm-b13-message-shell'>", unsafe_allow_html=True
    )
    st.markdown(
    ...
    for msg in messages:
        ...
        if st.button(
            ...
            if ok:
                set_system_message(
                    "Message archived. You can find it in Daily Food Journal → Nutritionist Notes Archive.",
                    "success",
                )
            else:
                set_system_message(
...
def _render_upcoming_schedules(user_id):
    ...
    for schedule in upcoming_schedules:
        ...
        notice_html = (
            f"<div class='hm-v101-schedule-line hm-v104b11-ack-note'>{_esc(notice)}</div>"
            if notice
            else ""
        )
        st.markdown(
...
def _home_action(label, page, key, disabled=False):
    st.markdown(
...
with right:
    with st.container(border=True):
        ...
        if not plans_ready:
            st.markdown(
                "<div class='lock-card'><b>Weekly plan, recipes, exercises and supplements unlock after expert review is complete.</b></div>",
                unsafe_allow_html=True,
            )
