...
.hm-v990-submit-note{color:#64748B;font-size:.80rem;font-weight:720;margin:.36rem 0 .20rem 0;}
...
div[data-testid="stHorizontalBlock"]:has(.hm-home-panel-anchor){align-items:stretch!important;}
div[data-testid="stHorizontalBlock"]:has(.hm-home-panel-anchor) > div[data-testid="column"]{align-self:stretch!important;background:rgba(255,255,255,.88)!important;border:1px solid #E9DFCC!important;border-radius:22px!important;box-shadow:0 14px 34px rgba(25,36,31,.08)!important;padding:1rem!important;}
div[data-testid="stHorizontalBlock"]:has(.hm-home-panel-anchor) > div[data-testid="column"] > div[data-testid="stVerticalBlock"]{height:100%!important;min-height:100%!important;}
...
@media(max-width:900px){div[data-testid="stHorizontalBlock"]:has(.hm-member-identity-pill){margin-top:-.55rem!important;}div[data-testid="stHorizontalBlock"]:has(.hm-home-panel-anchor) > div[data-testid="column"]{padding:.85rem!important;}}
</style>
""",
...
def _render_upcoming_schedules(user_id):
    ...
    if not upcoming_schedules:
        return
    st.markdown(
        "<div class='hm-nutritionist-message-shell'>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='hm-b13-message-title'>Upcoming Schedule</div>",
        unsafe_allow_html=True,
    )
    for schedule in upcoming_schedules:
        ...
        st.markdown(
            ...
            <div class='hm-v101-schedule-card'>
              <div class='hm-v101-schedule-title'>
                {_esc(schedule.get('title','Scheduled session'))}
                <span class='hm-v101-schedule-pill'>
                  {_esc(schedule_display_status_label_v104b11(schedule))}
                </span>
              </div>
              <div class='hm-v101-schedule-line'>
                {_esc(schedule.get('schedule_date',''))} · {_esc(time_text)}
              </div>
              <div class='hm-v101-schedule-line'>
                Mode: {_esc(schedule.get('mode','-'))} ·
                Link/location: {_esc(schedule.get('location_or_link') or '-')}
              </div>
              {notice_html}
            </div>
            """,
    ...
    st.markdown(
        ...
        <div class='hm-v990-task-progress'>
          <div class='hm-v990-progress-head'>
            <div class='hm-v990-progress-title'>
              Task progress: {progress_done} of {progress_total} completed
            </div>
            <div class='hm-v990-progress-due'>Due date: {due_date}</div>
          </div>
          <div class='hm-v990-progress-line'>
            <div class='hm-v990-progress-fill' style='width:{progress_width}%;'></div>
          </div>
          <div>{''.join(task_chips)}</div>
          <div class='hm-v990-submit-note'>
            Use Submit / Status after completing all requested tasks to send this
            to admin for review.
          </div>
        </div>
        """,
...
def _home_action(label: str, page: str, key: str, disabled: bool = False):
    st.markdown(
        "<span class='hm-home-action-anchor'></span>",
        unsafe_allow_html=True,
    )
    if st.button(
        label,
...
with left:
    st.markdown(
        "<span class='hm-home-panel-anchor'></span>",
        unsafe_allow_html=True,
    )
    st.subheader("Your next steps")
    is_task_instance = (
        current_instance.get("instance_type") in ["Task Request", "Reassessment"]
