...
from components.db import get_workflow, get_resource_assignments, save_resource_feedback, get_resource_feedback, get_published_recommendation_for_date
...
def _published_exercise_ids_for_member(member_id):
    share = get_published_recommendation_for_date(member_id)
    ids = []
...
def render_landing(df):
    ...
    if "hm_exercise_grid_page" not in st.session_state:
        st.session_state["hm_exercise_grid_page"] = 1
    ...
    if st.session_state.get("exercise_category_filter_v94_4", "All") not in category_options:
        st.session_state["exercise_category_filter_v94_4"] = "All"
    if st.session_state.get("exercise_difficulty_filter_v94_4", "All") not in difficulty_options:
    ...
    if st.session_state.get("hm_exercise_grid_page", 1) > max_page:
        st.session_state["hm_exercise_grid_page"] = max_page
    page = int(st.session_state.get("hm_exercise_grid_page", 1))
...
""", unsafe_allow_html=True)
                    ...
                    with ac1:
                        if st.button("View exercise", key=f"view_exercise_{idx}", type="secondary", use_container_width=True):
                            ...
                            st.session_state["hm_exercise_detail_mode"] = True
                            st.rerun()
                    with ac2:
        ...
        if total_results > page_size:
            ...
            with prev_col:
                if st.button("← Previous", use_container_width=True, disabled=(page <= 1), key="exercise_prev_page_v1024b7"):
                    st.session_state["hm_exercise_grid_page"] = max(1, page - 1)
                    st.rerun()
            with page_col:
                st.markdown(f"<div class='hm-page-indicator'>Page {page} of {max_page} · Showing {start_i + 1}-{end_i}</div>", unsafe_allow_html=True)
            with next_col:
                if st.button("Next →", use_container_width=True, disabled=(page >= max_page), key="exercise_next_page_v1024b7"):
                    st.session_state["hm_exercise_grid_page"] = min(max_page, page + 1)
                    st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
assigned_ids = set(published_plan_ids or list(get_resource_assignments(user_id, "exercises")))
if assigned_ids:
    df = df[df.index.astype(str).isin(assigned_ids)].copy()

selected_id = st.session_state.get("hm_exercise_selected_id")
is_detail_view = (
    bool(st.session_state.get("hm_exercise_detail_mode"))
...
if is_detail_view:
    ...
    with nav_back:
        if st.button("← Back to Exercise Repository", use_container_width=True, key="exercise_detail_back_to_repository_v1024b8"):
            st.session_state.pop("hm_exercise_selected_id", None)
            st.session_state.pop("hm_exercise_detail_mode", None)
            st.rerun()
    with nav_dashboard:
        if st.button("Dashboard", use_container_width=True, key="exercise_detail_dashboard_v1024b8"):
            st.session_state.pop("hm_exercise_selected_id", None)
            st.session_state.pop("hm_exercise_detail_mode", None)
...
# v102.4B6: Exercise repository spacing, 3-column grid and smoother filter transition.
st.markdown("""
<style>
/* v102.4B6 Exercise Repository compact layout */
.hero-shell{
...
# v102.4B6: Exercise Repository now uses compact 3-column member grid, reduced spacing, and single-rerun filter toggle.
# v102.4B8: detail view uses local Back to Repository so selected card state is cleared safely.
# v102.4B5: restored legacy v93+ Exercise Repository member UI while preserving Recommendations Share binding.
