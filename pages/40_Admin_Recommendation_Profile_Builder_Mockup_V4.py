...
@media(max-width:900px){.hm-grid,.hm-readiness{grid-template-columns:1fr}.hm-section-nav [data-testid="stButton"]>button{min-height:2.45rem!important;}}
</style>
""", unsafe_allow_html=True)
...
def item_row(kind, day, slot):
    ...
    for idx in range(count(key)):
        if kind == "meal":
            ...
            b.text_input("Time", key=f"{key}_time_{idx}", placeholder=SELECT_TIME)
