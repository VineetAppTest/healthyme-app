...
def item_rows(kind, day, slot, options, cols):
    ...
    for idx in range(count(key)):
        if kind == "meal":
            ...
            d.text_input("Instruction", key=f"{key}_instruction_{idx}", label_visibility="collapsed")
    button_label = {"meal": "Add food item", "exercise": "Add workout item", "supplement": "Add supplement item"}[kind]
    if st.button(button_label, key=f"add_{key}", use_container_width=True):
        add(key)
        st.rerun()
