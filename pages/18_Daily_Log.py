...
def _normalise_other_fluids(items):
    ...
    for item in items or []:
        if not isinstance(item, dict):
            continue
        cleaned = {"type": _clean(item.get("type") or item.get("name") or item.get("fluid_type")), "time": _clean(item.get("time")), "quantity": _clean(item.get("quantity") or item.get("qty")), "notes": _clean(item.get("notes"))}
...
def _render_exercise_journal_placeholder():
    ...
    st.markdown("""<div class='hm-exercise-placeholder'>Exercise Journal will be enabled after the exercise logging data contract is finalised. For now, please continue recording any exercise under <b>Food Journal → Physical Activity</b>.</div>""", unsafe_allow_html=True)
