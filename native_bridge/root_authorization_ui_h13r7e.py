...
AUTHORIZATION_BUILD = "H13R7E-meaningful-wellness-authorizer-v1"
ROLLBACK_BUILD = "H13R7D-minimal-wellness-authorizer-v1"
...
def render_root_authorization_ui(authorization_id: str) -> None:
    ...
    st.markdown(
        ...
        [data-testid="stAppViewContainer"] {
            background:
                radial-gradient(circle at 7% 4%, rgba(246,168,91,.14), transparent 22rem),
                radial-gradient(circle at 95% 10%, rgba(15,116,92,.11), transparent 23rem),
    ...
    <style>
      ...
      * {{ box-sizing:border-box; }}
      ...
      #hm-page {{
        width:100%;max-width:1240px;margin:0 auto;padding:8px 4px 24px;
      }}
      .hm-topbar {{
      ...
      .hm-brand-name {{
        font-size:32px;line-height:1;font-weight:850;letter-spacing:-.7px;color:var(--hm-ink);
      }}
      ...
      .hm-journey-card {{
        ...
        background:
          radial-gradient(circle at 88% 8%,rgba(207,157,66,.13),transparent 15rem),
          linear-gradient(145deg,#eef8f3 0%,#fffaf1 68%,#f8efe0 100%);
      }}
      .hm-journey-card::before {{ background:linear-gradient(90deg,var(--hm-green),#56a788); }}
      .hm-login-card h1,.hm-journey-card h2 {{ margin:0;color:var(--hm-ink); }}
      .hm-login-card h1 {{ font-size:25px;line-height:1.2;margin-bottom:9px; }}
      .hm-journey-card h2 {{ font-size:24px;line-height:1.2;margin-bottom:10px; }}
      ...
      .hm-journey-intro {{ margin-bottom:22px; }}
      .hm-path {{
        display:flex;flex-direction:column;gap:14px;flex:1;justify-content:center;
      }}
      .hm-path-card {{
        position:relative;display:flex;align-items:center;gap:16px;
        min-height:116px;padding:22px;border:1px solid rgba(15,114,93,.18);
        border-radius:16px;background:rgba(255,255,255,.80);
        box-shadow:0 10px 24px rgba(15,78,61,.06);
      }}
      .hm-path-card:first-child {{ border-left:5px solid var(--hm-coral); }}
      .hm-path-card:last-child {{ border-left:5px solid var(--hm-green); }}
      .hm-path-icon {{
        flex:0 0 auto;width:54px;height:54px;display:grid;place-items:center;
        border-radius:16px;background:var(--hm-mint-strong);color:var(--hm-green-dark);
        font-size:25px;
      }}
      .hm-path-card:first-child .hm-path-icon {{
        background:#fff0ec;color:var(--hm-coral-dark);
      }}
      .hm-path-card strong {{
        display:block;color:var(--hm-ink);font-size:16px;line-height:1.25;
      }}
      .hm-path-card span {{
        display:block;margin-top:6px;color:#64736c;font-size:12px;line-height:1.45;
      }}
      .hm-path-arrow {{
        align-self:center;width:34px;height:34px;display:grid;place-items:center;
        border-radius:50%;background:#fff;border:1px solid var(--hm-gold-soft);
        color:var(--hm-gold);font-size:18px;font-weight:800;
      }}
      .hm-privacy-note {{
        margin-top:20px;padding-top:16px;border-top:1px solid rgba(15,114,93,.14);
        color:#6a766f;font-size:11px;line-height:1.45;
      }}
      ...
      @media (max-width:900px) {{
        ...
        .hm-path {{ justify-content:flex-start; }}
      }}
      @media (max-width:620px) {{
        ...
        .hm-journey-card h2 {{ font-size:21px; }}
        .hm-copy,.hm-journey-copy {{ font-size:13px; }}
        .hm-field + .hm-field {{ margin-top:13px; }}
        .hm-path-card {{ min-height:auto;padding:18px;align-items:flex-start; }}
        .hm-path-icon {{ width:46px;height:46px;border-radius:14px;font-size:21px; }}
        .hm-path-card strong {{ font-size:15px; }}
        .hm-path-arrow {{ width:30px;height:30px;font-size:16px; }}
      }}
    </style>
    ...
    <main id="hm-page">
      ...
      <section class="hm-main-grid">
        ...
        <aside class="hm-card hm-journey-card">
          <div class="hm-journey-intro">
            <h2>Continue your HealthyMe journey</h2>
            <p class="hm-journey-copy">
              Sign in to resume your assessment and access the guidance prepared for you.
            </p>
          </div>

          <div class="hm-path">
            <div class="hm-path-card">
              <div class="hm-path-icon">✓</div>
              <div>
                <strong>Complete your assessment</strong>
                <span>Continue your Lifestyle and NSP assessment from where you left off.</span>
              </div>
            </div>

            <div class="hm-path-arrow">↓</div>

            <div class="hm-path-card">
              <div class="hm-path-icon">🌿</div>
              <div>
                <strong>View expert guidance</strong>
                <span>Access practical next steps shared for your wellness progress.</span>
              </div>
            </div>
          </div>

          <div class="hm-privacy-note">
            Your progress remains linked to your authorised HealthyMe account.
          </div>
        </aside>
      </section>
    </main>
