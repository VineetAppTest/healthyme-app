...
class MemberDailyLogSelectorAndDiagnosticsTests(unittest.TestCase):
    def test_daily_log_selector_highlights_multiple_streamlit_selected_states(self):
        source = (
            ROOT / "components/member_exercise_journal_table_bootstrap.py"
        ).read_text()
        self.assertIn('button[aria-pressed="true"]', source)
        self.assertIn('button[aria-checked="true"]', source)
        self.assertIn('button[data-selected="true"]', source)
    ...
    def test_selector_does_not_mutate_its_widget_key_after_creation(self):
        ...
        self.assertIn('selected = selected if selected in _DAILY_LOG_LABELS else', source)
    ...
    def test_disabled_measurement_uses_footer_panel_only(self):
        ...
        self.assertIn("if resolved_page.startswith(_MEMBER_PAGE_PREFIX) and (", source)
        self.assertIn("diagnostics.measurement_enabled() or isinstance(summary, dict)", source)
