...
def test_recipe_and_exercise_show_only_four_stable_sections():
    ...
    assert "hm-admin-content-section-inactive" in source


def test_legacy_feedback_and_allocation_are_hidden_not_deleted():
...
def test_success_reset_is_staged_before_next_widget_render():
    ...
    assert source.index("_apply_staged_reset()", source.index("def _render_selector")) < source.index(
        "st.columns(4", source.index("def _render_selector")
    )
