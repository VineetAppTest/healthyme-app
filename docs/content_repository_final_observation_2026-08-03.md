...
The canonical Supabase Content Repository is the sole live authority for Recipe, Exercise and Supplement definitions. The final gate removes the remaining no-op compatibility sync and legacy repository diagnostics, and moves the two retained CSV sources from active `data/` paths into a documentation archive.
...
- no `sync_repository_to_state` compatibility function;
- no `sync_all_repositories_to_state` compatibility function;
- no publishing-time compatibility sync call;
