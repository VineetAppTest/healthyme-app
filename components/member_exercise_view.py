...
def render_member_exercise_journal(key_prefix: str = "member_exercise") -> None:
    """Render the complete member exercise experience inline.

    The prescription remains read-only. Only status, completion time and member
    notes are written back to the member exercise-log contract.
    """
    ...
    for index, exercise in enumerate(exercises, start=1):
        ...
        notes = st.text_area(
            ...
            key=f"{key_prefix}_notes_{item_order}",
            height=88,
        )

        if st.button(
            "Save Progress",
            key=f"{key_prefix}_save_{item_order}",
