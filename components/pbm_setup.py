...
from components.pbm_core import (
    ...
    bump_epoch, clean, clean_date, load_selected, member_maps,
...
def render_setup(options) -> None:
    ...
    with left:
        profile["profile_name"] = st.text_input("Profile Name", value=profile.get("profile_name", ""), key=f"pbm_profile_name_{epoch}")
        ...
        if clone_columns[1].button("Clone Setup", use_container_width=True, disabled=not bool(clone_id)):
            ...
            if ok:
                ...
                st.session_state["pbm_profile"] = cloned; st.session_state["pbm_items"] = []; bump_epoch(); st.success("Profile Setup cloned. Recommendation rows were not copied."); st.rerun()
            st.error(clone_message)
        profile["change_note"] = st.text_input("Change Note", value=profile.get("change_note", ""), key=f"pbm_profile_change_note_{epoch}")
        st.text_input("Profile Status", value="Draft", disabled=True)
    with right:
        profile["region"] = st.text_input("Region / Food Culture", value=profile.get("region", ""), key=f"pbm_profile_region_{epoch}")
        ...
        profile["age_band"] = st.selectbox("Age Band", age_options, index=age_options.index(current_age), key=f"pbm_profile_age_band_{epoch}")
        ...
        profile["health_concerns"] = st.multiselect("Health Concerns", concerns, default=profile.get("health_concerns", []), key=f"pbm_profile_health_concerns_{epoch}")
        ...
        profile["diet_type"] = st.selectbox("Diet Type", diet_options, index=diet_options.index(current_diet), key=f"pbm_profile_diet_type_{epoch}")
    ...
    with lower_left:
        ...
        member_label = st.selectbox("Member Assignment", member_labels, index=member_labels.index(current_member), key=f"pbm_profile_member_{epoch}"); profile["assigned_member_label"] = member_label; profile["assigned_member_id"] = label_to_id.get(member_label, "")
        profile["profile_note"] = st.text_area("Profile-level Nutritionist Note", value=profile.get("profile_note", ""), height=150, key=f"pbm_profile_note_{epoch}")
    with lower_right:
        profile["start_date"] = st.date_input("Plan Start Date", value=clean_date(profile.get("start_date")), key=f"pbm_profile_start_date_{epoch}")
