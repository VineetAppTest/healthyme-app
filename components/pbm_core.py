...
APP_BUILD_VERSION = "v100.39"
APP_BUILD_LABEL = "Module-Specific Save Workflow"
...
SELECT_PROFILE = "-- Select draft profile --"
...
def clean_date(value: object) -> dt.date:
    ...
    try: return dt.date.fromisoformat(clean(value)[:10])
    except Exception: return dt.date.today()

def parse_dosage_frequency(value: object) -> Tuple[int, str]:
    raw = clean(value); match = re.match(r"^Frequency:\s*(\d+)\s*;\s*Dosage:\s*(.*)$", raw)
...
def parse_timeline(value: object) -> List[str]:
    if isinstance(value, list): return [item for item in value if item in SUPPLEMENT_TIMELINE]
...
def source_snapshot(kind: str, label: str) -> Dict[str, Any]:
    if not label or label.startswith("-- Select"): return {}
...
def image_reference(snapshot: Dict[str, Any]) -> str:
    image = as_dict(snapshot.get("image")); values = []
...
def timeline_from_source(value: object) -> List[str]:
    ...
    raw = clean(value).lower(); output: List[str] = []
    for tokens, option in [(("breakfast","morning"),"After Breakfast"), (("lunch","afternoon"),"After Lunch"), (("dinner","evening"),"After Dinner"), (("bed","night","sleep"),"Before Bed")]:
        if any(token in raw for token in tokens) and option not in output: output.append(option)
    return output
...
def normalise_item(row: Dict[str, Any]) -> Dict[str, Any]:
    item = dict(row or {}); item["ui_id"] = clean(item.get("id")) or uuid.uuid4().hex
    item["item_type"] = clean(item.get("item_type")); item["day_number"] = int(item.get("day_number") or 1)
    item["slot_name"] = clean(item.get("slot_name")); item["item_order"] = int(item.get("item_order") or 1)
    snapshot = as_dict(item.get("source_snapshot")); item["source_admin_overrides"] = as_dict(snapshot.get("admin_source_overrides"))
    if item["item_type"] == "supplement":
        item["frequency"], item["dosage"] = parse_dosage_frequency(item.get("dosage_frequency")); item["timeline"] = parse_timeline(item.get("scheduled_time"))
    return item

def profile_from_db(profile: Dict[str, Any]) -> Dict[str, Any]:
    return {"id": clean(profile.get("id")), "profile_name": clean(profile.get("profile_name")), "status": clean(profile.get("status"), "draft"), "region": clean(profile.get("region")), "age_band": clean(profile.get("age_band")) or SELECT_AGE, "diet_type": clean(profile.get("diet_type")) or SELECT_DIET, "health_concerns": list(profile.get("health_concerns") or []), "profile_note": clean(profile.get("profile_note")), "change_note": clean(profile.get("change_note")), "cycle_rule": clean(profile.get("cycle_rule"), "Weekly cyclical until replaced or stopped"), "assigned_member_id": clean(profile.get("assigned_member_id")), "assigned_member_label": clean(profile.get("assigned_member_label")) or SELECT_MEMBER, "start_date": clean_date(profile.get("start_date")), "clone_source_profile_id": clean(profile.get("clone_source_profile_id")), "clone_source_label": clean(profile.get("clone_source_label")) or "New profile"}
...
def load_selected(profile_id: str, shell_only: bool = False) -> Tuple[bool, str]:
    ...
    if ok: apply_loaded(profile, items, shell_only)
    return ok, message
...
def member_maps() -> Tuple[List[str], Dict[str, str], Dict[str, str], str]:
    options, message = load_member_options(); labels: List[str] = []; label_to_id: Dict[str,str] = {}; id_to_label: Dict[str,str] = {}
    for row in options:
        label, member_id = clean(row.get("label")), clean(row.get("id"))
        if label: labels.append(label); label_to_id[label] = member_id
        if member_id: id_to_label[member_id] = label
    if SELECT_MEMBER not in label_to_id: labels.insert(0, SELECT_MEMBER); label_to_id[SELECT_MEMBER] = ""
...
def rows_for(kind: str, day: int, slot: str) -> List[Dict[str, Any]]:
    ...
    if not rows: row = new_row(kind, day, slot); st.session_state["pbm_items"].append(row); rows = [row]
    rows.sort(key=lambda value: int(value.get("item_order") or 1))
    for index, row in enumerate(rows, 1): row["item_order"] = index
    return rows
...
def storage_rows(kind: str) -> List[Dict[str, Any]]:
    ...
    for source in st.session_state["pbm_items"]:
        if source.get("item_type") != kind: continue
        row = dict(source)
        if kind == "supplement": row["scheduled_time"] = ", ".join(row.get("timeline") or []); row["dosage_frequency"] = encode_dosage_frequency(int(row.get("frequency") or 0), clean(row.get("dosage")))
        if row_has_content(row): output.append(row)
    return output

def day_label(day: int) -> str:
    start = st.session_state["pbm_profile"].get("start_date"); start = start if isinstance(start, dt.date) else dt.date.today()
