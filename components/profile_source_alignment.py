...
def _count_cards(snapshot: Dict[str, Any]) -> None:
    ...
    recipe_missing = len(snapshot["recipe_missing_in_builder"])
    exercise_missing = len(snapshot["exercise_missing_in_builder"])
    supplement_missing = len(snapshot["supplement_missing_in_builder"])
    st.markdown(
        f"""
<div class='hm-count-grid'>
  ...
  <div class='hm-count-card'><b>{recipe_missing + exercise_missing + supplement_missing}</b><span>Source labels missing from builder dropdowns</span></div>
</div>
""",
...
def _coverage_table(snapshot: Dict[str, Any]) -> pd.DataFrame:
    recipe_not_pulled = [field for field in RECIPE_COLUMNS if field not in ["title", "portion_size"]]
    exercise_not_pulled = [field for field in EXERCISE_COLUMNS if field not in ["title"]]
    supplement_not_pulled = [field for field in SUPPLEMENT_REGIMEN_FIELDS if field not in ["supplement_name", "dosage", "frequency", "timing", "instructions"]]
    return pd.DataFrame([
        {
            "Repository": "Recipe Repository",
            "Current Profile Builder capture": ", ".join(PROFILE_BUILDER_CONTRACT["Recipe / Meal"]),
            "Important source fields not preserved yet": ", ".join(recipe_not_pulled),
            "Image references present": snapshot["recipe_images"],
            "Current judgement": "Not aligned — builder captures label/override only, not full recipe snapshot.",
        },
        {
            "Repository": "Exercise Repository",
            "Current Profile Builder capture": ", ".join(PROFILE_BUILDER_CONTRACT["Exercise"]),
            "Important source fields not preserved yet": ", ".join(exercise_not_pulled),
        ...
        },
        {
            ...
            "Current Profile Builder capture": ", ".join(PROFILE_BUILDER_CONTRACT["Supplement"]),
            "Important source fields not preserved yet": ", ".join(supplement_not_pulled),
...
def _missing_table(snapshot: Dict[str, Any]) -> pd.DataFrame:
    ...
    for source_key, label in [
        ...
        if not missing:
            rows.append({"Source": label, "Missing count": 0, "Examples": "None"})
            continue
        rows.append({
            "Source": label,
