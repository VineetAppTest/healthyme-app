...
_MARKER = "_hm_member_journal_server_autosave_v1"
...
def _should_autosave_exercise(button_key: str) -> bool:
    ...
    if signature == baseline:
        return False
    st.session_state[baseline_key] = signature
    st.session_state["_hm_last_journal_autosave"] = "exercise"
    return True


def install_member_journal_server_autosave() -> None:
    """Autosave committed Daily Log widget changes through existing save paths.

    The wrapper is intentionally server-side. It does not use JavaScript, timers,
    DOM scanning or synthetic browser clicks. The existing Save Day and Save
    Progress handlers remain the only persistence authorities.
    """
