...
def browser_has_legacy_supabase_marker() -> bool:
    """Detect the marker left by PR #128 without executing a browser component.

    The marker is never trusted for authentication. It is used only to prevent a
    stale Auth0 admin browser identity from taking over a failed member recovery.
    """
    try:
...
def restore_supabase_login_from_session(force_refresh: bool = False) -> bool:
    """Restore an already-resolved Supabase identity from Streamlit session state.

    This intentionally avoids the PR #128 process-local cookie registry, which
    could not survive a Render/app restart and caused component flicker/crashes.
    """
    ...
    if not app_user:
        if st.session_state.get("logged_in") and st.session_state.get("_hm_auth_role_resolved"):
            st.session_state["auth_error"] = (
                f"{email or 'This Supabase user'} role refresh could not be confirmed. "
                "Existing resolved session retained for this request."
            )
            return False
    ...
    try:
        ...
        if not app_user:
            st.session_state["auth_error"] = (
                ...
                "HealthyMe. Confirm hm_users.role and hm_users.auth_user_id/email mapping."
            )
