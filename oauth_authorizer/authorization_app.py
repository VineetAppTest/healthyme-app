...
BUILD = "H13Q4-supabase-oauth-authorizer-v1"
...
client_login_url = _secret(
    "CLIENT_LOGIN_URL",
    "https://healthyme-native-role-bridge.streamlit.app/Login",
)
authorization_id = str(st.query_params.get("authorization_id") or "").strip()

st.title("HealthyMe Supabase authorization")
st.caption(
    "This app performs only the Supabase sign-in and OAuth authorization step. "
    "It does not create a Streamlit native identity, resolve a HealthyMe role, "
    "render a protected page or perform logout."
)
st.code(BUILD)

if not authorization_id:
    st.warning(
        "The authorization request is missing or has expired. Start a fresh login "
        "from the HealthyMe native role router."
    )
    st.link_button(
